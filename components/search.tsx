...
const mapDispatchToProps = (dispatch: Dispatch) => ({
  ...
  setSelectedArtSlug: (slug: string) =>
    dispatch(action.setSelectedCategorySlug(slug)),
});
    ...
    <>
      <div className="search-bar">
        ...
        <input
          ...
          placeholder="Search categories, artists available for collaboration"
          aria-label="search"
          value={inputVal}
          onChange={(event) => {
            resetState();
        ...
        ></input>
        {inputVal.length > 0 && (
          <button className="search-bar__cross" onClick={clearInput}>
                ...
                entityType === "ART" ? (
                  <div key={i} className="typeahead-item">
                    <div onMouseDown={handleSearchClick(href, data)}>
                      <span className="typeahead-item__name common-text-style">
                        {name}
                      </span>
                      <span className="typeahead-item__category common-text-style">
                        {entityType}
                      </span>
                    </div>
                  </div>
          ...
          <div className="typeahead-container">
            {
              <div key={1} className="typeahead-item">
                <div>
                  <span className="typeahead-item__name common-text-style">
                    Oops, no result found. Try something else.
                  </span>
                </div>
              </div>
            }
